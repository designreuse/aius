					...
					<div class="col-lg-12">
						<div class="ibox float-e-margins" style="margin-bottom: 10px;">
							...
							<div class="ibox-content"
								...
								${vo.at_content}</div>
							<div class="ibox-content" style="padding-top: 10px; padding-bottom: 9px;">
				...
				<div class="row">
					<div class="col-lg-12">
						...
						<c:if test="${id ne null}">
							...
							<a href="/sReply?b=${b}&p=${p}&n=${n}"id="reply"><button type="button"
											class="btn btn-default btn-xs btn-left" id="sReply">답글</button></a>	


							<c:if test="${level ne null && nickname ne null && id ne null}">
								<c:if test="${level >= '9'|| nickname eq vo.at_writer}">
									<a href="/sModify?b=${b}&p=${p}&n=${n}"><button type="button"
											class="btn btn-default btn-xs btn-left">수정</button></a>
								</c:if>
								<c:if
									test="${ (level >= '9'|| nickname eq vo.at_writer) && vo.at_deleted eq 'N'}">
									<a href="#"><button type="button"
											class="btn btn-default btn-xs btn-left" id="sDelete" data-a="${b}" data-b="${vo.at_id}">삭제</button></a>
								</c:if>
								<c:if test="${level >= '9' && vo.at_deleted eq 'Y'}">
									<a href="#"><button type="button"
											class="btn btn-default btn-xs btn-left" id="atRestore" data-a="${b}" data-b="${vo.at_id}">복구</button></a>
								</c:if>
							</c:if>
						</c:if>
					</div>
